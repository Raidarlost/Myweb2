...
export default function BeatCard({ beat }: { beat: Beat }) {
  ...
  return (
    <div className="group relative rounded-2xl bg-dark-card border border-dark-border hover:border-gold/40 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] hover:-translate-y-1 transition-all duration-300 overflow-hidden">
      {/* Cover Art */}
  ...
  {beat.duration && <span>{formatDuration(beat.duration)}</span>}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-dark-border">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg btn-gold text-xs font-semibold hover:scale-105 transition-transform duration-200"
          >
            Add to Cart
          </button>
        </div>
